...
class Cluster:
    def __init__(self, timed_transactions):
        self.timed_transactions=timed_transactions
        self.D, self.W, self.H, self.S, self.G, self.Occ, self.Ci=0,0,0,0,0,[], 0
    def calc(self, total_objs):

        objs=[o for tr in self.timed_transactions for o in tr[1:]]
        unique_objs=set(objs)
        self.D=len(unique_objs)
        self.Occ=np.bincount(objs, minlength=total_objs)
        self.S=sum(self.Occ)
        self.W=len(unique_objs)
        self.H=self.S/self.W
        self.G=self.H/self.W
        self.Ci=len(self.timed_transactions)
    def to_str(self):
        return f"D={self.D}, Occ={self.Occ}, S={self.S}, W={self.W}, H={self.H}, Tr={self.timed_transactions}"

def calc_profit(clusters):
    a=sum(c.G*c.Ci for c in clusters if len(c.timed_transactions))
    b=sum(c.Ci for c in clusters)
    return a/b
    
def run_clope_iter(all_transactions,  total_objs, clusters=[]):
    timed_transactions=[[f"t={i}", *tr] for i, tr in enumerate(all_transactions)]

    if len(clusters)==0:
        clusters=[Cluster([tr]) for tr in timed_transactions]
        for c in clusters: c.calc(total_objs)
        return clusters
    else:
        
        #very_max_profit_prev, very_max_profit=0, 0

        clusters_=clusters

        while True:
            num_tr1=sum(len(c.timed_transactions) for c in clusters_)

            moved=False
            #very_max_profit_prev=very_max_profit
            for tr in timed_transactions:
                clusters_=[*clusters, Cluster([])]

                for c in clusters_:
                    if tr in c.timed_transactions:
                        c_prev=c
                        break
                c_best, max_profit=None, calc_profit(clusters_)
                
                for i,c in enumerate(clusters_):
                    #удаляем транзакцию из предыд. кластера, чтоб в кажд. момент врем. она была только в одном из ник
                    c_prev.timed_transactions.remove(tr)
                    c.timed_transactions.append(tr)
                    c_prev.calc(total_objs), c.calc(total_objs)
                    profit=calc_profit(clusters_)
                    print(f"Checking profit: {profit}/{max_profit}")
                    if profit>max_profit:
                        c_best, max_profit=c, profit
                        if c!=c_prev: moved=True
                        c_prev=c
                        #if max_profit>very_max_profit:
                            #very_max_profit=max_profit
                    else: 
                        c.timed_transactions.remove(tr)
                        c_prev.timed_transactions.append(tr)
                        c_prev.calc(total_objs), c.calc(total_objs)
            #if very_max_profit==very_max_profit_prev:
                #break
            num_tr2=sum(len(c.timed_transactions) for c in clusters_)

            assert num_tr1==num_tr2, "Error"
            print(f"Num transactions = {num_tr1}")

            if not moved: break
        
        res=[c for c in clusters_ if len(c.timed_transactions)]
        return res
        ...
        for ev in pygame.event.get():
            ...
            if ev.type == pygame.KEYDOWN:
                ...
                if ev.key==pygame.K_4:
                    clusters=run_clope_iter(all_transactions, len(objs)-1, clusters)
