...
class Graph:
    ...
    def find_route_2_robots(self, objs, n1A, n2A, n1B, n2B):
        all_nodes=self.get_all_nodes()
        for n in all_nodes: 
            n.visitedA, n.DA, n.routeA = False, 100500, []
            n.visitedB, n.DB, n.routeB = False, 100500, []
        n1A.DA, n1A.routeA, waveA = 0, [n1A], [n1A]
        n1B.DB, n1B.routeB, waveB = 0, [n1B], [n1B]
        vA, vB = None, None
        L0 = dist(n1A.get_pos(), n1B.get_pos())
        c=np.mean([n2A.get_pos(), n2B.get_pos()], axis=0)
        def H(n, n2): #эвристика
            e = dist(n.get_pos(), c) # 1 - евклидово расстояние
            inters = any([o.intersects([n.get_pos(), n2.get_pos()]) for o in objs]) # 2 - факт столкновения
            dl = (dist(n.get_pos(), n2.get_pos()) - L0)**2 # 3 - разрыв балки
            err = e/800 + 10000*int(inters) + dl/100
            return err
        while True:
            vA, vB = None, None
            if len(waveA):vA=waveA.pop(np.argmin([n.DA for n in waveA]))
            if len(waveB):vB=waveB.pop(np.argmin([n.DB for n in waveB]))
            if vA is not None:
                vA.visitedA, nextA = True, [e for e in vA.edges if not e.n2.visitedA]
                for e in nextA: #если текущ. вершина vA лучше соседней вершины e.n2, то обновл. соседа
                    if e.n2.DA + H(e.n2, vB) > vA.DA + H(vA, vB)  + e.w: 
                    #if e.n2.DA  > vA.DA   + e.w: 
                        e.n2.DA, e.n2.routeA=vA.DA+e.w, vA.routeA+[e.n2]
                    if not e.n2 in waveA: waveA.append(e.n2) #берем соседа в фронт поиска
            if vB is not None:
                vB.visitedB, nextB = True, [e for e in vB.edges if not e.n2.visitedB]
                for e in nextB: #если текущ. вершина vB лучше соседней вершины e.n2, то обновл. соседа
                    if e.n2.DB + H(e.n2, vA) > vB.DB + H(vB, vA) + e.w: 
                    #if e.n2.DB  > vB.DB  + e.w: 
                        e.n2.DB, e.n2.routeB=vB.DB+e.w, vB.routeB+[e.n2]
                    if not e.n2 in waveB: waveB.append(e.n2) #берем соседа в фронт поиска
            if len(waveA)+len(waveB)==0: break
        return list(reversed(n2A.routeA)), list(reversed(n2B.routeB))
...
if __name__=="__main__":
    ...
    route=None
    route2A, route2B=None, None
    debug_node=None

    line=[[400, 300], [500, 400]]

    n1A = graph.find_nearest_node(robot.get_pos())
    n1B = graph.find_nearest_node(robot2.get_pos())
    ...
    while True:
        for ev in pygame.event.get():
            ...
            if ev.type == pygame.KEYDOWN:
                if ev.key==pygame.K_1:
                    route=graph.find_route(n1A,n2A)
                if ev.key==pygame.K_2:
                    route2A, route2B=graph.find_route_2_robots(objs, n1A, n2A, n1B, n2B)
        ...
        if all(r is not None for r in (route2A, route2B)):
            for ir, route in enumerate((route2A, route2B)):
                for i in range(len(route)):
                    pygame.draw.line(screen, (255,0,255), route[i-1].get_pos(), route[i].get_pos(), [4,2][ir])
