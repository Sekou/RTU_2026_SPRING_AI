...
# проверка удовлетвориельности решения в сравнении ранее найденным минимальным путем
    def check_upper_bound(perm, L_upper, pts): 
        s=0
        for i in range(len(perm)-1):
            s+=dist(perm[i], perm[i+1])
            L_lower=s + estimate_lower_bound(perm[i+1:], pts) #NEW perm[i+1:] вместо perm[:i+1]
            #print(L_lower, L_upper)
            if L_lower>=L_upper: return False, perm[:i+2]
        return True, None

    def estimate_lower_bound(part, pts): #уточняем нижнюю оценку намеченного маршрута (что пройдено + что осталось)
        p_left=[p for p in pts if not p in part]
        buf=[pts[part[-1]], *p_left] # в буфере первых точек: последняя из пройденного маршрута и все непройденные
        res=0
        while len(buf):
