#NEW 2 - поправил направления слайсов, граничные индексы, имена перменных вложенных циклов, обрезку ветвей "ok1 and"
# в ф-ции check_upper_bound_for_current_path
# в ф-ции estimate_lower_bound_for_remaining_path
...
def branch_and_bound_searсh(pts, ind):  # применение метода ветвей и границ
    ...
    def filter_bad_trunks(perm): #учитиываем лексикографическое хранение, чтоб удалить старые записи
        nonlocal bad_trunks #регистр отсекаемых ветвей получается распределен во времени
        bad_trunks=[bt for bt in bad_trunks if consistent(bt,perm)]

    # [132, 4561, 23, ...]
    ndel, nadd=0,0
    for step, perm in enumerate(perms):  # [2345]
        if step%1000==0: 
            print(f"Step: {step + 1}, added {nadd} branches, skipped {ndel} branches")
            ndel, nadd=0,0
        ...
        if not ok1:  # ветвь уже запрещена - уходим на след. итерацию
            ...
            L = calc_path_len(path)
            ndel+=1
            if DEBUG: print(f"Skipping deleted branch: ind+perm={ind}+{perm}, path={path}, L={L}")
            continue
        ok2, trunk = check_upper_bound_for_current_path(ind, perm, L_upper, pts)  # NEW

        # ветвь не запрещена, но верхняя граница не соблюдается - добавляем запрет и уходим на след. итерацию
        if not ok2:
            ...
            filter_bad_trunks(perm)

            continue

        path = [pts[ind]] + [pts[i] for i in perm]
        L = calc_path_len(path) #РЕСУРСОЕМКАЯ ОПЕРАЦИЯ, КОТОРУЮ СТРЕМИМЧЯ ПРОПУСКАТЬ

        if L < min_len:
            min_len, min_path, L_upper = L, path, L
            print("Found better path")

    print("BAD TRUNKS: ", bad_trunks)
    return min_path
