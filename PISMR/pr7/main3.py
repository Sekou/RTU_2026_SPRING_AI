...
def branch_and_bound_searсh(pts, ind): # применение метода ветвей и границ
    ...
    def check_upper_bound(ind, perm, L_upper, pts, debug=False): 
        ...
        for i in range(len(perm)-1):
            ...
            visited_pts=[pts[i] for i in perm[:i+2]] #NEW2
            L_lower=s + estimate_lower_bound(visited_pts, pts, debug) #NEW perm[i+1:] вместо perm[:i+1]
            #print(L_lower, L_upper)
            #if L_lower>=L_upper: return False, perm[:i+2]
            if L_lower>L_upper: return False, perm[:i+1] #NEW2
        return True, None

    def estimate_lower_bound(visited_pts, pts, debug=False): # уточняем нижнюю оценку оставшегося маршрута
    ...
    for step,perm in enumerate(perms): #[2345]
        ...
        if not ok1:  #ветвь уже запрещена - уходим на след. итерацию
            ...
            L=calc_path_len(path)
            if L<763:
                print(ind, "+", perm) #4 + (0, 9, 1, 6, 8, 2, 7, 3, 5) #наикратчайший путь из 4 точки
                print(path) #[(150, 150), (100, 100), (100, 200), (150, 200), (150, 300), (250, 300), (250, 200), (200, 150), (250, 100), (350, 100)]
                print(L, " / ", L_upper) #762.1320343559643  / 906.1208747436233
                print(bad_trunks) #[(0,)]
                ok2, trunk = check_upper_bound(ind, perm, L_upper, pts, debug=True)
                print(ok2)
            ...
            continue
        ok2, trunk = check_upper_bound(ind, perm, L_upper, pts) #NEW
