...
class Robot:
    ...
    def get_conf(self): return [self.steer, self.speed]
    def clear(self):
...
class RRTNode:
    def __init__(self, steer, speed, prev_pt=None, x=0, y=0, ang=0): # конфигурации (steer, speed) вместо точек (x, y)
        self.steer, self.speed, self.prev_pt, self.next_confs = steer, speed, prev_pt, []
        self.x, self.y, self.ang=x,y,ang #для 1. сопоставления с целевой точкой и 2. моделирования робота
    def get_pos(self): return [self.x, self.y]
    def draw(self, screen): 
    ...
    def gen_next_conf(self, robot, target_conf, save=True): #вместо gen_next_pt
        ...
        #моделируем робота на шаге RRT
        delta=rot([n.speed*RRT_DTIME, 0], self.ang)
        n.x, n.y, n.ang=self.x+delta[0], self.y+delta[1], self.ang
        if n.steer!=0:
            R = robot.L/n.steer
            da = n.speed*RRT_DTIME/R
            n.ang=lim_ang(n.ang+da)

        if save: self.next_confs.append(n)
        return n

#отличие от run_RRT_iter_test - в том, что отрезки никогда не пересекутся
def run_RRT_iter(nodes, robot, goal, objs):
