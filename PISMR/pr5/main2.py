...
def merge_nodes(route):
    res=[]
    i=0
    while i<len(route):
        if i<len(route)-1 and dist(route[i-1].get_pos(), route[i].get_pos())<RRT_STEP/2:
            res.append(RRTNode(*np.add(route[i].get_pos(), route[i+1].get_pos())/2))
            i+=2
        else:
            res.append(route[i])
            i+=1
...
def backtrace_route(node):
    res=[node]
    while node.prev_pt is not None:
        res.append(node.prev_pt)
        node=node.prev_pt
    return res
    

def run_RRT_Connect_iter(nodes1, nodes2, start, goal, objs):
    if len(nodes1)==0: nodes1[:]=[RRTNode(*start)]
    if len(nodes2)==0: nodes2[:]=[RRTNode(*goal)]

    arr = [(dist(n1.get_pos(), n2.get_pos()), n1, n2) for n1 in nodes1 for n2 in nodes2]
    arr=sorted(arr, key = lambda v: v[0])
    d, n1_, n2_= arr[0]

    if d <  RRT_STEP:
        return True, backtrace_route(n1_)[::-1]+backtrace_route(n2_)
    run_RRT_iter2(nodes1, goal, objs)
    run_RRT_iter2(nodes2, start, objs)
    d=dist(nodes1[-1].get_pos(), nodes2[-1].get_pos())
    if d < RRT_STEP:
        return True, backtrace_route(n1_)[::-1]+backtrace_route(n2_)
    return False, []
    ...
    while True:
        for ev in pygame.event.get():
            ...
            if ev.type==pygame.KEYDOWN: 
                if ev.key==pygame.K_1: run_RRT_iter(nodes, goal, objs)
                ...
                if ev.key==pygame.K_4: 
                    flag, route2 = run_RRT_Connect_iter(nodes, nodes2, start, goal, objs)

        dt=1/fps
        screen.fill((255, 255, 255))
        for obj in objs: obj.draw(screen)
        for n in nodes: n.draw(screen)
        for n in nodes2: n.draw(screen)
        if len(route): 
            pygame.draw.lines(screen, (0,0,255), False, [n.get_pos() for n in route], 2)

        if len(route2): 
            pygame.draw.lines(screen, (0,0,255), False, [n.get_pos() for n in route2], 4)
